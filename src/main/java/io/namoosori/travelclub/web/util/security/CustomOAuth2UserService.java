...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        httpSession.setAttribute("member", new MemberCdo(memberJpo.toDomain())); // SessionUser (직렬화된 dto 클래스 사용)

        return new DefaultOAuth2User(Collections.singleton(new SimpleGrantedAuthority(memberJpo.toDomain().getRoleKey())),
    ...
    private MemberJpo saveOrUpdate(OAuthAttributes attributes){
        ...
        if(memberJpo == null){
            MemberJpo memberJpo1 = new MemberJpo();
            memberJpo1.setName(attributes.getName());
            memberJpo1.setPhoneNumber(attributes.getPhoneNumber());
            memberJpo1.setEmail(attributes.getEmail());
